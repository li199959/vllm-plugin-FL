...
class OpManager:
    ...
    def __init__(self, registry: Optional[OpRegistry] = None) -> None:
        ...
        # Cache for resolve_candidates(): (op_name, policy, epoch) -> ordered list
        self._candidates_cache: Dict[Tuple[str, "SelectionPolicy", int], list] = {}
    ...
    def _resolve_impl(self, op_name: str) -> OpImpl:
        """Resolve and cache the best OpImpl for an op (internal).

        Returns the full OpImpl so callers can read both ``fn`` and ``impl_id``
        without an extra registry walk.  Cache is keyed by the policy object
        itself (it is a frozen dataclass) plus the policy epoch.
        """
    ...
    def _compute_candidates(self, op_name: str, policy: "SelectionPolicy") -> list:
        """Build the filtered candidate list for an op (no ordering applied).

        Centralised so resolve() and resolve_candidates() share the same
        vendor/availability filtering logic.
        """
        snap = self._registry.snapshot()
    ...
    def resolve_candidates(self, op_name: str) -> list[OpImpl]:
        ...
        The result list is cached per ``(op_name, policy, epoch)`` so the
        fallback path is no slower than the strict path on warm calls.
        ``failed_impls`` is filtered at call time (see ``call``), not here,
        so the cache stays valid across failures.
    ...
    def call(self, op_name: str, *args, **kwargs):
        ...
        if not enable_fallback:
            # Strict mode: single resolve, no fallback.  Resolve once to get
            # the OpImpl so we can read both fn and impl_id without a second
            # registry walk.
