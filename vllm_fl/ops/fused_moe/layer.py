...
# Detect vLLM version: v0.16+ refactored FusedMoE (no select_experts)
try:
    from vllm.model_executor.layers.fused_moe import FusedMoE as _FusedMoEBase
    _HAS_SELECT_EXPERTS = hasattr(_FusedMoEBase, "select_experts")
except ImportError:
    _HAS_SELECT_EXPERTS = False
...
if _HAS_SELECT_EXPERTS:
    ...
    try:
        from vllm.model_executor.layers.fused_moe.router.fused_topk_bias_router import (
            fused_topk_bias as _fused_topk_bias,
        )
    except ImportError:
        from vllm.model_executor.layers.fused_moe.fused_moe import (
            fused_topk_bias as _fused_topk_bias,
        )
    grouped_topk = _grouped_topk
    fused_topk_bias = _fused_topk_bias
    ...
    FusedMoE.select_experts = FusedMoEFL.select_experts

else:
    # v0.16+: FusedMoE was completely refactored (no select_experts method).
    # Skip MoE patch entirely; use vLLM's built-in implementation.
    UnquantizedFusedMoEMethodFL = None
    FusedMoEFL = None
    UnquantizedFusedMoEMethod = None
    FusedMoE = _FusedMoEBase
